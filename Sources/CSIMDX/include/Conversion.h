...
CX_INLINE Float32x2 CXFloat32x2FromFloat64x2(Float64x2 operand) {
    ...
    #pragma clang loop vectorize(enable) interleave(enable)
     for (size_t i = 0; i < 2; i++) {
        result.rawValue[i] = (Float32)(operand.rawValue[i]);
    }
#endif
    return result;
}
...
CX_INLINE Float32x2 CXFloat32x2FromInt64x2(Int64x2 operand) {
    Float32x2 result;
#if CX_ARM_ARCH >= CX_NEON_LEVEL_AARCH64
    result.rawValue = vcvt_f32_f64(vreinterpretq_f64_s64(operand.rawValue));
#else
    #pragma clang loop vectorize(enable) interleave(enable)
     for (size_t i = 0; i < 2; i++) {
        result.rawValue[i] = (Float32)(operand.rawValue[i]);
...
CX_INLINE Float32x2 CXFloat32x2FromUInt64x2(UInt64x2 operand) {
    Float32x2 result;
#if CX_ARM_ARCH >= CX_NEON_LEVEL_AARCH64
    result.rawValue = vcvt_f32_f64(vreinterpretq_f64_u64(operand.rawValue));
#else
    #pragma clang loop vectorize(enable) interleave(enable)
     for (size_t i = 0; i < 2; i++) {
        result.rawValue[i] = (Float32)(operand.rawValue[i]);
...
CX_INLINE Int32x2 CXInt32x2FromFloat64x2(Float64x2 operand) {
    Int32x2 result;
#if CX_ARM_ARCH
    result.rawValue = vcvt_s32_f32(vcvt_f32_f64(operand.rawValue));
#else
    #pragma clang loop vectorize(enable) interleave(enable)
     for (size_t i = 0; i < 2; i++) {
        result.rawValue[i] = (Int32)nearbyint(operand.rawValue[i]);
    }
#endif
    return result;
}
...
CX_INLINE Int32x2 CXInt32x2FromInt64x2(Int64x2 operand) {
    Int32x2 result;
#if CX_ARM_ARCH
    result.rawValue = vmovn_s64(operand.rawValue);
#else
    #pragma clang loop vectorize(enable) interleave(enable)
     for (size_t i = 0; i < 2; i++) {
        result.rawValue[i] = (Int32)(operand.rawValue[i]);
    }
#endif
    return result;
}
...
CX_INLINE Int32x2 CXInt32x2FromUInt64x2(UInt64x2 operand) {
    Int32x2 result;
#if CX_ARM_ARCH
    result.rawValue = vreinterpret_s32_u32(vmovn_u64(operand.rawValue));
#else
    #pragma clang loop vectorize(enable) interleave(enable)
     for (size_t i = 0; i < 2; i++) {
        result.rawValue[i] = (Int32)(operand.rawValue[i]);
...
CX_INLINE UInt32x2 CXUInt32x2FromFloat64x2(Float64x2 operand) {
    UInt32x2 result;
#if CX_ARM_ARCH
    result.rawValue = vcvt_u32_f32(vcvt_f32_f64(operand.rawValue));
#else
    #pragma clang loop vectorize(enable) interleave(enable)
     for (size_t i = 0; i < 2; i++) {
        result.rawValue[i] = (UInt32)nearbyint(operand.rawValue[i]);
    }
#endif
    return result;
}
...
CX_INLINE UInt32x2 CXUInt32x2FromInt64x2(Int64x2 operand) {
    UInt32x2 result;
#if CX_ARM_ARCH
    result.rawValue = vreinterpret_u32_s32(vmovn_s64(operand.rawValue));
#else
    #pragma clang loop vectorize(enable) interleave(enable)
     for (size_t i = 0; i < 2; i++) {
        result.rawValue[i] = (UInt32)(operand.rawValue[i]);
    }
#endif
    return result;
}
...
CX_INLINE UInt32x2 CXUInt32x2FromUInt64x2(UInt64x2 operand) {
    UInt32x2 result;
#if CX_ARM_ARCH
    result.rawValue = vmovn_u64(operand.rawValue);
#else
    #pragma clang loop vectorize(enable) interleave(enable)
     for (size_t i = 0; i < 2; i++) {
        result.rawValue[i] = (UInt32)(operand.rawValue[i]);
...
CX_INLINE Float64x2 CXFloat64x2FromFloat32x2(Float32x2 operand) {
    Float64x2 result;
#if CX_ARM_ARCH >= CX_NEON_LEVEL_AARCH64
    result.rawValue = vcvt_f64_f32(operand.rawValue);
#else
    #pragma clang loop vectorize(enable) interleave(enable)
     for (size_t i = 0; i < 2; i++) {
        result.rawValue[i] = (Float64)(operand.rawValue[i]);
    }
#endif
    return result;
}
...
CX_INLINE Float64x2 CXFloat64x2FromInt32x2(Int32x2 operand) {
    Float64x2 result;
#if CX_ARM_ARCH >= CX_NEON_LEVEL_AARCH64
    result.rawValue = vcvt_f64_f32(vreinterpret_f32_s32(operand.rawValue));
#elif CX_X86_ARCH >= CX_X86_LEVEL_SSE2
    result.rawValue = _mm_cvtpi32_pd(operand.rawValue);
#else
    #pragma clang loop vectorize(enable) interleave(enable)
     for (size_t i = 0; i < SIZE(result); i++) {
        result[i] = (Float64)(operand[i]);
    }
#endif
    return result;
}
...
CX_INLINE Float64x2 CXFloat64x2FromUInt32x2(UInt32x2 operand) {
    Float64x2 result;
#if CX_ARM_ARCH >= CX_NEON_LEVEL_AARCH64
    result.rawValue = vcvt_f64_f32(vreinterpret_f32_u32(operand.rawValue));
#elif CX_X86_ARCH >= CX_X86_LEVEL_AVX_512
    result.rawValue = _mm_cvtepu32_pd(_mm_movpi64_epi64(operand.rawValue));
#else
    #pragma clang loop vectorize(enable) interleave(enable)
     for (size_t i = 0; i < 2; i++) {
        result.rawValue[i] = (Float64)(operand.rawValue[i]);
...
CX_INLINE Int64x2 CXInt64x2FromFloat32x2(Float32x2 operand) {
    Int64x2 result;
#if CX_ARM_ARCH >= CX_NEON_LEVEL_AARCH64
    result.rawValue = vcvtq_s64_f64(vcvt_f64_f32(operand.rawValue));
#else
    #pragma clang loop vectorize(enable) interleave(enable)
     for (size_t i = 0; i < 2; i++) {
        result.rawValue[i] = (Int64)(operand.rawValue[i]);
    }
#endif
    return result;
}
...
CX_INLINE Int64x2 CXInt64x2FromInt32x2(Int32x2 operand) {
    Int64x2 result;
#if CX_ARM_ARCH >= CX_NEON_LEVEL_AARCH64
    result.rawValue = vmovl_s32(operand.rawValue);
#elif CX_X86_ARCH >= CX_X86_LEVEL_SSE4_1
    result.rawValue = _mm_cvtepi32_epi64(_mm_movpi64_epi64(operand));
#else
    #pragma clang loop vectorize(enable) interleave(enable)
     for (size_t i = 0; i < 2; i++) {
        result.rawValue[i] = (Int64)(operand.rawValue[i]);
...
CX_INLINE Int64x2 CXInt64x2FromUInt32x2(UInt32x2 operand) {
    Int64x2 result;
#if CX_ARM_ARCH >= CX_NEON_LEVEL_AARCH64
    result.rawValue = vreinterpretq_s64_u64(vmovl_u32(operand.rawValue));
#elif CX_X86_ARCH >= CX_X86_LEVEL_SSE4_1
    result.rawValue = _mm_cvtepi32_epi64(_mm_movpi64_epi64(operand.rawValue));
#else
    #pragma clang loop vectorize(enable) interleave(enable)
     for (size_t i = 0; i < 2; i++) {
        result.rawValue[i] = (Int64)(operand.rawValue[i]);
...
CX_INLINE UInt64x2 CXUInt64x2FromFloat32x2(Float32x2 operand) {
    UInt64x2 result;
#if CX_ARM_ARCH >= CX_NEON_LEVEL_AARCH64
    result.rawValue = vcvtq_u64_f64(vcvt_f64_f32(operand.rawValue));
#elif CX_X86_ARCH >= CX_X86_LEVEL_AVX_512
    result.rawValue = _mm_cvtps_epu64(operand.rawValue);
#else
    #pragma clang loop vectorize(enable) interleave(enable)
     for (size_t i = 0; i < 2; i++) {
        result.rawValue[i] = (UInt64)(operand.rawValue[i]);
    }
#endif
    return result;
}
...
CX_INLINE UInt64x2 CXUInt64x2FromInt32x2(Int32x2 operand) {
    UInt64x2 result;
#if CX_ARM_ARCH >= CX_NEON_LEVEL_AARCH64
    result.rawValue = vreinterpretq_u64_s64(vmovl_s32(operand.rawValue));
#elif CX_X86_ARCH >= CX_X86_LEVEL_SSE4_1
    result.rawValue = _mm_cvtepi32_epi64(_mm_movpi64_epi64(operand.rawValue));
#else
    #pragma clang loop vectorize(enable) interleave(enable)
...
CX_INLINE UInt64x2 CXUInt64x2FromUInt32x2(UInt32x2 operand) {
    UInt64x2 result;
#if CX_ARM_ARCH >= CX_NEON_LEVEL_AARCH64
    result.rawValue = vmovl_u32(operand.rawValue);
#elif CX_X86_ARCH >= CX_X86_LEVEL_SSE4_1
    result.rawValue = _mm_cvtepi32_epi64(_mm_movpi64_epi64(operand.rawValue));
#else
    #pragma clang loop vectorize(enable) interleave(enable)
